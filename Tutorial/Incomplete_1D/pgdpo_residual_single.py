...
# ===== (2) Antithetic runtime: simulator & (reused) comparison =====
from pgdpo_antithetic_single import (
    simulate_antithetic,                                 # domain-time + true antithetic
    print_policy_rmse_and_samples_antithetic as compare_policy_functions,  # 재사용
)
...
# ========================= Stage-1: residual PG-DPO (always antithetic) =========================
def train_residual_stage1(
    *,
    ...
    - Simulator: domain-time, per-path τ with Δt=τ/m
    - Always TRUE antithetic (+/- noise with same local seed)
    - Logs: loss only (no E[U] prints)
    """
    ...
    for ep in range(1, epochs + 1):
        ...
        # 에폭마다 로컬 시드 변주 (TRUE antithetic은 같은 시드의 ±잡음쌍)
        pair_seed = None if seed_train is None else int(seed_train) + ep

        # 정책 유틸리티 (안티테틱 평균) — grad 필요
        U_pol = simulate_antithetic(pol, batch_size, train=True, seed_local=pair_seed)
        loss = - U_pol.mean()
...
def main():
    # 공용 러너 재활용:
    #  - 학습: residual (myopic base) + antithetic (seed로 고정)
    #  - 평가: 3번의 compare 함수를 재사용 (CRN_SEED_EU, REPEATS/SUBBATCH 적용)
    run_common(
