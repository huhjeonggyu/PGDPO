# pgdpo_residual_single.py
# Version 5 (updated):
# - Single asset, single factor
# - Euler–Maruyama simulator
# - Stage-1 residual learning policy: pi = pi_myopic(Y) + hedge(W, TmT, Y)
...
# - No antithetic sampling, no Richardson extrapolation, no control variates
# Requirements: torch, numpy
#
# Usage:
#   python pgdpo_residual_single.py
...
class HedgeNet(nn.Module):
    def __init__(self):
        ...
        self.lamb = nn.Parameter(torch.tensor(0.0))  # learnable decay

    def forward(self, W, TmT, Y):
        x = torch.cat([W, TmT, Y], dim=1)
        decay = torch.exp(-torch.exp(self.lamb) * TmT)  # short-horizon emphasis
        h = self.net(x) * decay
        return torch.tanh(h)
...
# ------------------------- Comparisons -------------------------
@torch.no_grad()
def compare_policy_functions(trained_policy: nn.Module, proj_policy: nn.Module, cf_policy: nn.Module):
    ...
    print(f"[Policy RMSE] Stage-1 (residual) vs CF: {rmse_s1_cf:.6f}")
...
def compare_expected_utility(stage1_policy: nn.Module, cf_policy: nn.Module, proj_policy: nn.Module=None):
    ...
    print(f"[EU]  E[U]_Stage-1 (residual) = {EU_s1:.6f}")
...
def main():
    ...
    # Stage 1: train residual policy
    trained = train_stage1()
    with torch.no_grad():
        U_s1 = simulate(trained, batch_size, train=False).mean().item()
    print(f"[After Train] E[U] Stage-1 policy: {U_s1:.6f}")

    # Stage 2: build projector
    projector = PPGDPOPolicy(trained, repeats=eval_repeats, sub_batch=sub_repeat).to(device)

    # 1) Policy-function RMSE @ τ=T
    compare_policy_functions(trained, projector, cf_policy)

    # 2) Expected-utility comparison under common random numbers
    compare_expected_utility(trained, cf_policy, projector if EVAL_PPGDPO_EU else None)
