...
# - Stage-1 residual learning policy (pi = pi_myopic + hedge) trained with Control Variates
...
#     (ii) Expected utility under common random numbers: Stage-1 vs CF  [PPGDPO EU optional & heavy]
# - No antithetic sampling, no Richardson extrapolation
# Requirements: torch, numpy
#
# Usage:
#   python pgdpo_cv_single.py
...
# Control Variates
CV_USE_EMA = True
CV_EMA_BETA = 0.9  # smoothing for c-hat
...
@torch.no_grad()
def myopic(Y):
    # single-asset myopic demand: (1/gamma) * (mu - r)/sigma^2, with mu - r = sigma*(alpha*Y)
    return (1.0/gamma) * (alpha * Y) / sigma  # [B,1]
...
class HedgeNet(nn.Module):
    def __init__(self):
        ...
        self.lamb = nn.Parameter(torch.tensor(0.0))  # short-horizon decay
...
class ResidualPolicy(nn.Module):
    def __init__(self): super().__init__(); self.hedge = HedgeNet()
    def forward(self, W, TmT, Y):
        pi = myopic(Y) + self.hedge(W, TmT, Y)
...
# -------------- Control policy & analytic expectation ----------
@torch.no_grad()
def analytic_U_of_wealth(WT):
    if abs(gamma - 1.0) < 1e-8:
        return WT.log()
    else:
        return (WT.pow(1.0 - gamma)) / (1.0 - gamma)

@torch.no_grad()
def analytic_EU_M(W0, TmT):
    # Control policy: riskless (pi_M=0) -> W_T = W0 * exp(r * (T - t0))
    WT = W0 * torch.exp(r * TmT)
    return analytic_U_of_wealth(WT)

# -------------------- Stage 1: Train with CV ------------------
def train_stage1_with_cv():
    policy = ResidualPolicy().to(device)
    opt = optim.Adam(policy.parameters(), lr=lr)

    c_hat_ema = torch.tensor(0.0, device=device)

    for ep in range(1, epochs+1):
        opt.zero_grad()

        # Sample initial states (needed for CV normalization & E[U_M])
        W0, Y0 = sample_initial_states(batch_size)
        TmT0 = torch.full_like(W0, T)

        # Main utility
        U_theta = simulate(policy, batch_size, train=True, W0=W0, Y0=Y0, Tval=T)

        # Control variate: pi_M=0 -> deterministic wealth path; use analytic expectation
        with torch.no_grad():
            U_M_sim   = analytic_U_of_wealth(W0 * torch.exp(r * TmT0))
            EU_M_true = analytic_EU_M(W0, TmT0)

            # Normalize by W0^(1-gamma) for stability
            scale = W0.pow(1.0 - gamma) if abs(gamma-1.0) >= 1e-8 else torch.ones_like(W0)
            U_theta_n = U_theta / scale
            U_M_cen_n = (U_M_sim - EU_M_true) / scale

            # Batch coefficient c = Cov/Var with small eps
            Utn = U_theta_n.view(-1)
            Umn = U_M_cen_n.view(-1)
            Umn_mean = Umn.mean()
            Utn_mean = Utn.mean()
            cov = ((Utn - Utn_mean) * (Umn - Umn_mean)).mean()
            var = ((Umn - Umn_mean)**2).mean()
            c_batch = cov / (var + 1e-12)
            if CV_USE_EMA:
                c_hat_ema = CV_EMA_BETA * c_hat_ema + (1.0 - CV_EMA_BETA) * c_batch
                c_use = c_hat_ema.detach()
            else:
                c_use = c_batch.detach()

        # Variance-reduced loss
        loss = - ( (U_theta_n - c_use * U_M_cen_n).mean() )
        loss.backward()
        ...
        if ep % 25 == 0 or ep == 1:
            ...
            print(f"[{ep:04d}] loss={loss.item():.6f}  E[U]_policy={U_est:.6f}  c={c_use.item():.4f}")
    return policy

# ------------- Stage 2: costates via BPTT (no tricks) ---------
def estimate_costates(policy_for_sim: nn.Module, W_pts: torch.Tensor, Y_pts: torch.Tensor,
    ...
    """
    Returns lambda_hat=dU/dW0, dlam_dW=d^2U/dW0^2, dlam_dY=d^2U/(dW0 dY0).
    (CV is only for training stability; costates computed from raw path utility.)
    """
    n = W_pts.size(0)
    ...
    while done < repeats:
        ...
        lam = lam.view(rpts, n, 1).mean(dim=0)
        ...
        lam_sum += lam.detach(); dW_sum += dlamW.detach(); dY_sum += dlamY.detach()
        done += rpts
...
def compare_policy_functions(stage1_policy: nn.Module, proj_policy: nn.Module, cf_policy: nn.Module):
    ...
    print(f"[Policy RMSE] Stage-1 (CV) vs CF:  {rmse_s1_cf:.6f}")
...
def compare_expected_utility(stage1_policy: nn.Module, cf_policy: nn.Module, proj_policy: nn.Module=None):
    ...
    if EVAL_PPGDPO_EU and proj_policy is not None:
        ...
        U_pp = simulate_with_noise(proj_policy, W0, Y0, dBW, dBY)
        EU_pp = U_pp.mean().item()
        print(f"[EU]  E[U]_P-PGDPO = {EU_pp:.6f}")
...
def main():
    ...
    # Stage 1: train residual policy with CV
    stage1 = train_stage1_with_cv()
    with torch.no_grad():
        U_s1 = simulate(stage1, batch_size, train=False).mean().item()
    print(f"[After Train] E[U] Stage-1 policy (CV): {U_s1:.6f}")
    ...
    # 2) Expected-utility comparisons (common random numbers)
